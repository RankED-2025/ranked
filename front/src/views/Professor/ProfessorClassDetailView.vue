...
const studentProgressionsByCourse = computed<Record<number, StudentCourseProgression[]>>(() => {
  ...
  for (const course of assignedCourses.value) {
    if (!progressionsByCourse[course.id]) {
      progressionsByCourse[course.id] = []
    }
  ...
  <div class="class-detail-view">
    <v-container class="py-8">
      ...
      <template v-else-if="classDetail">
        ...
        <v-card
          v-if="assignedCourses.length === 0"
          elevation="1"
          rounded="lg"
          class="text-center pa-8 mb-6"
        >
          ...
          <div class="text-body-2 text-grey mb-4">
            Assignez un cours à cette classe pour suivre la progression des élèves.
          </div>
          <v-btn
            color="primary"
            variant="tonal"
                ...
                <template #append>
                  <div class="d-flex align-center gap-2" style="min-width: 180px">
                    ...
                    <v-icon
                      ...
                      size="20"
                      :color="
                        student.badge.type === 'gold'
                          ? 'amber-darken-2'
                          : student.badge.type === 'silver'
                            ? 'grey'
                            : 'deep-orange'
                      "
